...
local function deparseR(exp, t, kind)
	...
	elseif obj(exp) == '"' then
		...
		if const then
			...
			local text = table.concat(r) --string.char(table.unpack(r))

			t[#t+1] = string.format('%q', text):gsub('\n', 'n')
	...
	elseif isfn(exp) then
		...
		elseif binop[op] and isobj(exp.a) then
			...
			t[#t+1] = op
			t[#t+1] = ' '
			deparseR(exp.a[2], t, true)
			if kind then t[#t+1] = ')' end

		elseif unop[op] then
			t[#t+1] = op
			t[#t+1] = ' '
			deparseR(exp.a, t, true)
...
function deparse(exp)
	local t = {}
	combal = {}
	deparseR(exp, t, false)
	return table.concat(t)
end

-- deparse maar 1 level
function deparsesimpel(exp)
	if isatom(exp) then
		return deparse(exp)
	else
		local nep = {}
		...
		for k,sub in subs(exp) do
			nep[k] = (isatom(sub) and sub) or (sub.ref and X(sub.ref)) or X'...'
		end
		nep.exp = nil
		return deparse(nep)
	end
