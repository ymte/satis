...
local function compopt0(exp, maakindex)
	...
	if fn(a) == '_fn' and fn(b) == '_fn' then
		...
		local nbody = clone(abody)

		local bbbody = clone(bbody)
		local bbbody = substitute(bbbody, X('_arg', barg), carg)
		local bbbody = substitute(bbbody, X('_arg0', barg), carg0)
		local bbbody = substitute(bbbody, X('_arg1', barg), carg1)
		local bbbody = substitute(bbbody, X('_arg2', barg), carg2)
		local bbbody = substitute(bbbody, X('_arg3', barg), carg3)

		local cbody = substitute(nbody, X('_arg', aarg), bbbody)
