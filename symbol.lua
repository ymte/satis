...
function substitute(exp, van, naar, klaar)
	...
	if isatom(exp) then
		...
		for k,v in subs(exp) do
			t[k],m = substitute(v, van, naar, klaar)
			n = n + m
		end
		klaar[exp] = t
		return t, n
...
if test then
	local a = X('+', 'a', 'a')
	local s = X('/', '1', '2')
	local b = substitutezuinig(a, X'a', s, makevars())
	local _, tel = string.gsub(hash(b), "/", "")
	assert(tel == 1, hash(b))
